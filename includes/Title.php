...
class Title {
	...
	public function getFullURL( $query = '', $query2 = false, $proto = PROTO_RELATIVE ) {
        ...
        global $wgWikiname,$wgIsUgcWiki,$wgPhpServer,$wgServer;
        $url = str_replace( "/$wgWikiname/$wgWikiname", "/$wgWikiname", $url);
        Hooks::run( 'GetFullURL', array( &$this, &$url, $query ) );
        
        if($wgIsUgcWiki){
        	//ugc wiki  预览url 需要跳转  wiki.joyme
        	$url = str_replace($wgPhpServer, $wgServer, $url);
        }else{
        	//数字站wiki 预览／查看 只能在本域名下
        	$url = str_replace($wgServer, $wgPhpServer, $url);
        }
	...
	public function getInternalURL( $query = '', $query2 = false ) {
		global $wgInternalServer, $wgServer,$wgSquidServer;
		$query = self::fixUrlQueryArgs( $query, $query2 );
		$server = $wgInternalServer !== false ? $wgInternalServer : $wgServer;
//		$server = $wgInternalServer !== false ? $wgInternalServer : $wgSquidServer;
