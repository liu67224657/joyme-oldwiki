...
class SkinTemplate extends Skin {
	...
	//替换掉action的地址， 统一指向 wiki php域名
    function getPhpUrl($str){
       global $wgServer,$wgPhpServer;
       return str_replace( $wgPhpServer, $wgServer, $str);
    }
    function getViewUrl($str){
       global $wgPhpServer;
       return $wgPhpServer.$str;
    }
		...
		if ( $out->isArticle() && $title->exists() ) {
			if ( $this->isRevisionCurrent() ) {
				if ( $wgMaxCredits != 0 ) {
					...
					$tpl->set( 'lastmodtime', date('Y-m-d H:i',strtotime($this->getOutput()->getRevisionTimestamp())+8*3600) );
				}
			}
