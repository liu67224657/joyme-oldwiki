...
class User implements IDBAccessObject {
	...
	public function jmuc_register(){
		global $com,$joyme_u_adminid;
		JoymeUser::initByRequest();
		$userInfo = JoymeUser::getUserInfo($com); 
		if($userInfo == false){
			return false;
		}
	
		$userInfo['nick'] = trim($userInfo['nick'], " ");
		$userInfo['nick'] = ucfirst(str_replace('_', ' ', $userInfo['nick']));
		$userInfo['nick'] = ucfirst(str_replace('#', ' ', $userInfo['nick']));
	
		$newUser = $this->createNew($userInfo['nick'],array('id'=>$userInfo['uid'],'password'=>':B:cc284d78:5cdade62046fb7d1d956944ce6b941f3','token'=>'172f1b14d720510f55fa58ec15f59a6f'));
		if($newUser){
			if(in_array($userInfo['uid'], $joyme_u_adminid)){
				$dbw = wfGetDB( DB_MASTER );
				$data = array(
						'ug_user' => $userInfo['uid'],
						'ug_group' => 'bureaucrat',
				);
	...
	private function getEditTokenAtTimestamp( $salt, $request, $timestamp ) {
		if ( $this->isAnon() ) {
			...
			$token = empty($_COOKIE['jmuc_token'])?'':$_COOKIE['jmuc_token'];
