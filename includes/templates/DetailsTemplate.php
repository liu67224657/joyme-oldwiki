<?php
class DetailsTemplate extends BaseTemplate {
    ...
    public function __construct($data,$filepath){
       ...
       $this->article = $data;
        $this->url = $wgServer;;
	   if(in_array("bureaucrat",$wgUser->mGroups)){
            $this->del = 'operation';
        }else{
            $this->del = '';
        }
    ...
    public function execute() {
		global $com, $wgThread, $wgIsLogin,$wgServer,$wgStaticUrl;
		if(!$wgThread){
            ...
            <div class="joyme-rmsy-tab-box">
                ...
                <div class="joyme-rmsy-tab-tit">
					...
					<?php if($this->article['power'] || (!$wgIsLogin && ($this->article['user_id'] == $_COOKIE['jmuc_u']))):?>
						<a href="<?php echo $wgServer.'/index.php?title='.$this->article['parse']['title'].'&action=edit';?>">编辑</a>
					<?php endif;?>
		...
		<script src="<?php echo $wgWikiStatic;?>/extensions/jsscripts/commonfn.js"></script>
		<script src="<?php echo $wgWikiStatic;?>/extensions/jsscripts/taolunqu_comment.js"></script>
		<script type="text/javascript" src="http://api.joyme.<?php echo $com;?>/json/mood?callback=emojiData"></script>
