...
HTML
	...
	protected function showStandardInputs( &$tabindex = 2 ) {
		global $wgOut,$wgAllowTagLibrary,$wgWikiname;
		$wgOut->addHTML( "<div class='editOptions'>\n" );

		if(in_array($wgWikiname,$wgAllowTagLibrary)){

			$model = new DataSynchronization();
			$lableHtml = $model->tagLibrary();
			$wgOut->addWikiText('<b>标签库:</b>');
			//标签库
			$wgOut->addHTML('<div id="category_labels_content" style="display:block;">'.$lableHtml.'</div>');
			$wgOut->addWikiText('单击添加标签');
			//已选标签
			$wgOut->addWikiText('<b>添加标签</b>');
			$wgOut->addHTML('<div id="choice_labels" class="choice_labels_content" style="width: 80%; height: 64px; line-height: 34px; border: 1px; background-color: rgb(255, 255, 255);"></div>');
			$wgOut->addWikiText('单击取消标签');
		}
	...
	public function getCheckboxes( &$tabindex, $checked ) {
		...
		//query the label content
//		$model = new SpecialLabelManagement();
//		$lableHtml = $model->tagLibrary();
		...
		if ( $wgUser->isLoggedIn() ) {
			...
			//标签库
//			$checkboxes['category_labels'] = Xml::check('btn_category_labels','',array('id'=>'btn_category_labels'))."标签库";
//
//			$checkboxes['tag_library'] = '<div id="category_labels_content" style="display:none;">'.$lableHtml.'</div>';
		}
