...
public function getHTMLTitle() {
        ...
        if ($this->getPageTitle() == '首页' || $this->getPageTitle() == '%E9%A6%96%E9%A1%B5' || urlencode('$this->getPageTitle()') == '%CA%D7%D2%B3' ||
            ...
            return $this->mHTMLtitle . '_着迷网Joyme';
        } else {
    ...
    public function getHTMLTitleRewrite() {
        ...
        if (!empty($wgSiteGameTitle)) {
            $mTitle = $this->getPageTitle() . '_' . $wgSiteGameTitle;
        } elseif (count($HTMLTitle_arry) > 1) {
            $mTitle = $this->getPageTitle() . '_' . $HTMLTitle_arry[(count($HTMLTitle_arry) - 1)];
        } else {
            $mTitle = $HTMLTitle;
        }
        return $mTitle . '_着迷网Joyme';
    }
	...
	public function headElement( Skin $sk, $includeStyle = true ) {
		global $wgContLang, $wgMimeType,$wgSiteSEOKeywords,$wgSiteSEODescription, $wgServer;
		...
		if($wgDefaultDevice !='pc'){
			$ret .= Html::element('meta', array('content' => 'width=device.width, initial-scale=1.0, user-scalable=no','name'=>'viewport')) . "\n";
	...
	function getHeadScripts() {
		...
		global $wgWikiname,$wgLoadphpCache,$wgServer,$wgPhpServer,$wgWikiCom;
		if($wgLoadphpCache){
			$wgLoadphpCacheStr = "true";
		}else{
			$wgLoadphpCacheStr = "false";
		}
		$links[] = Html::inlineScript('window.wgWikiCom="'.$wgWikiCom.'";window.wgPhpServer ="'.$wgPhpServer.'";window.wgLoadphpCache ='.$wgLoadphpCacheStr.';window.wgWikiname = "'.$wgWikiname.'";window.articleId = '.$tmpdata['wgArticleId'].';window.wgServer ="'.$wgServer.'";');
