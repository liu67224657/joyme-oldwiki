...
class Joyme {
    ...
    static function upload($operations) {
        ...
        $split_arr = explode('.', $_SERVER['HTTP_HOST']);
        $key1 = str_replace('mwstore://local-backend/local-public', 'wiki/images/'.$split_arr[0], $operations[1]['dst']);
        $key = str_replace('mwstore://local-backend/local-public', 'wiki/images/'.$split_arr[0],$operations[0]['dst']);
        //If there are historical records
        if($info){
            //如果是恢复
            if($operations[1]['op']=="copy"){
                //1.把原图移动到$operations[0]['src']目录
                $oldimg = str_replace('mwstore://local-backend/local-public', 'wiki/images/'.$split_arr[0],$operations[0]['dst']);
                $newach = str_replace('mwstore://local-backend/local-public', 'wiki/images/'.$split_arr[0],$operations[0]['src']);

                Qiniu_Utils::Qiniu_MoveFile($wgQiNiuBucket, $newach, $wgQiNiuBucket,$oldimg );

                $operations1 = str_replace('mwstore://local-backend/local-public', 'wiki/images/'.$split_arr[0],$operations[1]['src']);
	...
	function commentUpload(){
        ...
        global $wgQiNiuBucket,$wgQiNiuPath,$wgWikiname;
...
//				$msg = $err->Err ? $err->Err : '图片上传失败';
			...
			return "<script>parent.upImgCallback('".json_encode($res)."')</script>";
		}else{
