...
class SkinOp extends SkinTemplate {
	...
	public function initPage( OutputPage $out ) {
		global $wgLocalStylePath,$wgWikiStatic;
		...
		$out->addHeadItem( 'csshover',
			'<!--[if lt IE 7]><style type="text/css">body{behavior:url("' .
				...
				"'.$wgWikiStatic.'/vector/csshover{$min}.htc\")}</style><![endif]-->"
		);
...
?>
	...
	<div id="wrap">
		<div class="wiki_header">
			...
			?>
			<div class="mw-tit"><span>海贼王WIKI</span></div>
			<?php } else { ?>
			<div class="mw-tit"><span style="float:left"><?php $this->html( 'title' ) ?></span>
				...
				<div id="head-comments" style="float:left;"><span style="text-indent:10px; line-height:80px; font-size:14px;"><a id="totalcomments" href="#post_reply">(0条评论)</a></span></div>
				<?php endif;?>
			</div>
			<?php } ?>
			<!--mw-tit==end-->
			...
			<div id="content" class="mw-body" role="main">
				...
				<div id="bodyContent">
					...
					<div id="footer" role="contentinfo"<?php $this->html( 'userlangattributes' ) ?>>
						<?php $plink = $this->getFooterLinks();
						if(!empty($plink['info'])){
						$nlink['info'][0] = $plink['info'][0];
						 foreach ($nlink  as $category => $links ) { ?>
	...
	<div id="right-swap">
		...
		<div id="right-swap-btn" class="state-open">隐藏</div>
	</div>
	...
	<script type="text/javascript">
	    ...
	    /* 添加自定义滚动条 
        $('.result-box').perfectScrollbar({theme:'light'});
	    $('.right-swap-con').perfectScrollbar({theme:'light'});
		*/
		...
		var tabTools={
				...
				'tab':function(){
					var tabBtn=$('.timeAside span');
					var tabMain=$('.hide');
					tabBtn.each(function(i) {
						$(this).click(function(){
							$(this).addClass('sel').siblings('span').removeClass();	
							$(tabMain[i]).show().siblings('div').hide();
						})
					});
				}
			}
		...
		var tabToole={
			...
			//滚动
			'soll':function(){
				var box=$('.wp_TabBtnList');
				var soll=$('.wp_TabBtnList div');
				var btnLeft=$('.wp_btn_left');
				var btnRight=$('.wp_btn_right');
				var box_Width=box.width();
				var soll_Width=0;
				//获取内容的宽度
				$('.wp_TabBtnList span').each(function(e){
					soll_Width=soll_Width+$(this).width();
					//点击切换
					$(this).click(function(){
							$(this).addClass('on').siblings('span').removeClass();
							$($('.wp_TabBox')[e]).show().siblings('div.wp_TabBox').hide();	
						});
				});
				//右边
				btnRight.on('click',function(){
					soll.stop();
					if(soll_Width<=box_Width){}//若内容宽度大于盒子宽度
					else{
						var sLeft=soll.position().left;
						if(box_Width>=soll_Width+sLeft){								
							}else{
								soll.stop(false,true).animate({left:parseInt(sLeft-100)},500);
							}
						}
					});
				//左边
	...
	tabTools.int();
    twoDimensionCode();
    function getYearMonth_1214(judge){
		var now=new Date();
       ...
       if(opinion==='day'){
       	return str
       }else if(opinion==='week'){
       	return week
       }else{
		return year+'-'+month+'-'+date;
	   }
	}
	var objyear = document.getElementById('year_1224') || null ;
	objyear != null ? objyear.innerHTML=getYearMonth_1214() : '';
	var objweek = document.getElementById('week_1224') || null ;
	objweek != null ? objweek.innerHTML=getYearMonth_1214() : '';
	var objday = document.getElementById('day_1224') || null ;
	objday != null ? objday.innerHTML=getYearMonth_1214() : '';
	</script>
	<!-- Piwik -->
<script type="text/javascript">
  var _paq = _paq || [];
