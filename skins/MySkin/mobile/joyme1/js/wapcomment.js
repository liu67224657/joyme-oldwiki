...
// wiki文章页点评论跳转
function toComment(){
	var url = window.wgPhpServer + '/skins/MySkin/mobile/wapcomment.php?wgTitle='+wgTitle;
	location.href = url;
	return;
}
...
function getCommentListCallBack(commentObj, unikey, domain, ishot) {
	...
	var html = "<div id='"+hotid+"cont_cmt_list_" + reply.reply.rid + "' class='module-list' "+
                ...
                "<div class='module-txt'>"+ reply.reply.body.text +"</div>"+(window.wikipage ? '' : reCommentListHtml)+
                "<div class='module-date'><span>"+ reply.reply.post_date +"</span></div></div></div>"+"</div>";
    return html;
}
            ...
            if (resMsg.rs == '-1') {
				...
				if(oid==0){// 主楼评论
					...
					var commentHtml = "<div id='cont_cmt_list_" + result.reply.rid + "' class='module-list'><div class='module-infobox'>"+
						...
						"<div class='module-more' onclick='moreReply(this)'></div></div></div>"+
						"<div class='module-date'><span>"+ result.reply.post_date +"</span></div></div></div></div>";
            ...
            if (resMsg.rs == '-1') {
                ...
                if (oid == 0) {
					...
					if($("#all-comment div").length<1){// 没有评论
						...
						$("#hot-tit, #hot-comment").hide();
						$(".info-bar.all-tit").append($("#hot-tit > a"));
					}
                } 
            } else {
				if(undefined != commenterror[resMsg.rs]){
