...
class opTemplate extends BaseTemplate {
	...
	public function execute() {
		global $wgVectorUseIconWatch,$wgDefaultDevice, $wgIsLogin, $wgJoymeUserInfo;
...
?>		
		...
		if(!stristr($_SERVER['REQUEST_URI'], '%E9%A6%96%E9%A1%B5') && !strstr($_SERVER['REQUEST_URI'], ':') 
			&& !strstr($_SERVER['REQUEST_URI'], 'action') && $wgComment){
			echo '<script src="/extensions/jsscripts/WebViewComment.js?0.3"></script>';
			echo '<script src="/skins/MySkin/app/js/appcomment.js?0.3"></script>';
			// echo '<script>window.wikiUrl = "'.$_GET['wikiurl'].'";</script>';
			// echo '<div class="wp-comment"><a href="javascript:;">我要吐槽</a><span onclick="share()"></span></div>';
			echo '<div class="bottomBar"><a href="javascript:void(0);" onclick="initPostMask(this)"><span>评论</span></a><a href="javascript:void(0);" onclick="share(this)"><span>分享</span></a></div>';
			
		}
		?>
		<script>
			function share(obj){
				var shareNum = $(obj).children('span').html()=='分享' ? 0 : parseInt($(obj).children('span').html());
				$(obj).children('span').html(++shareNum);//console.log($(obj).children('span').html());
				wikiShare();
				var title = $("#firstHeading span[dir='auto']").text()=='' ? '海贼迷' : $("#firstHeading span[dir='auto']").text();
				var picurl = $("#share-img img").length>0 ? $("#share-img img").attr("src") : 'http://joymeapp.qiniudn.com/haizeimi74X74.png';
				var content = $("#share-content").length>0 ? $("#share-content").text() : '海贼世界的解密、追番双料神器海贼迷App，专属视频、图文、资讯，从此一网打尽！';
				var url = document.location.href;
				// console.log('title',title);
				// console.log('pic',picurl);
				// console.log('desc',content);
				// console.log('url',url);
				var stringval = 'title=' + encodeURI(title) + "&url=" + encodeURI(url) + "&picurl=" + encodeURI(picurl) + "&content="+encodeURI(content);
				// console.log('data',stringval);
				if (browser.versions.ios) {
					_jclient.callHandler('share', stringval, function(response) {
						alert("share->server=>" + response)
					})
				}
				if (browser.versions.android) {
					_jclient.share(stringval);
				}
			}
		</script>
		<!-- 图片查看 -->
		...
		<?php $this->printTrail(); ?>
		<script type="text/javascript" src="/skins/MySkin/app/op/js/swiper.js"></script>
		<script type="text/javascript" src="/skins/MySkin/app/op/js/common.js"></script>
		<!-- Piwik -->
...
<noscript><p><img src="//stat.joyme.com/piwik.php?idsite=6" style="border:0;" alt="" /></p></noscript>
<!-- End Piwik Code -->
	</body>
</html>
<?php
