...
function wikiShare(){
	$.ajax({
        url: window.joymeapi.api+"jsoncomment/reply/sharereport",
        type: "post",
        async: false,
        data: {unikey:uniKey, domain:domain, jsonparam:JSON.stringify(jsonParam)},
        dataType: "jsonp",
        jsonpCallback: "commentsharecallback",
    ...
    $.ajax({
        ...
        success: function (req) {
            ...
            if (resMsg.rs == '1') {
				...
				// 热门
				if ((result.hotlist == null || result.hotlist.length == 0) && (result.mainreplys.page ==null ||result.mainreplys.page.firstPage)) {
				...
				if(window.wikipage){
					...
					$(".bottomBar span:eq(1)").html(resMsg.result.share_sum);
...
function initPostMask(obj) {
	if($(obj).attr('data') == 'top'){
		var url = window.wgPhpServer + '/skins/MySkin/app/appcomment.php?wgTitle='+wgTitle;
		location.href = url;
		return;
	}
    if (!window.joymeIsLogin) {
