...
function checkUploadWarning( filename, fileItem ) {
	var mwVersion = parseInt( mw.config.get('wgVersion').substr( 2, 2 ) );
	if ( mwVersion > 21 ) {
		if(mw.config.get('wgServer') && mw.config.get('wgServer').indexOf(window.location.host) == -1){
	        var apiserver = window.wgPhpServer;
	    }else{
	        var apiserver = mw.config.get('wgServer');
	    }
		$.ajax({ url: apiserver + mw.util.wikiScript( 'api' ), dataType: 'json', type: 'POST',
		data: {
			format: 'json',
			action: 'query',
...
function createUpload( wikiEditor ) {
	...
	var uploader = new plupload.Uploader({
		...
		'url': apiserver + msuVars.path + '/../../api.php',
		'flash_swf_url': apiserver + msuVars.path + '/plupload/Moxie.swf',
		'silverlight_xap_url': apiserver + msuVars.path + '/plupload/Moxie.xap'
	});
